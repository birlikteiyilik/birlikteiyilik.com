...
export const artGeometry=(portrait:boolean)=>({left:portrait?120:1120,top:portrait?1080:180,width:portrait?840:700,height:portrait?740:720});
...
export const ArtBox:React.FC<{children:React.ReactNode}>=({children})=>{
  const{portrait}=useLayout();
  return <div style={{position:'absolute',...artGeometry(portrait)}}>{children}</div>;
};
